...
impl ProtocolHandler {
    ...
    pub const MAX_DATA_LENGTH: usize = 4; // Maximum
    pub const HALT_COMMAND: u8 = 0xc1;
    pub const RESUME_COMMAND: u8 = 0xc2;
    pub const READ_COMMAND: u8 = 0xc3;
    ...
    pub fn write_frame(&self) -> Vec<u8> {
        ...
        match &self.command {
            ...
            SWDCommand::Read { start_address, length } => {
                ...
                data.push(0x00); // Length (high byte)
                data.push(0x00); // Length (low byte)
                data.push(Self::READ_COMMAND); // Command
    ...
    pub fn read_frame(&self, data: &[u8]) -> Result<Vec<u8>, String> {
        ...
        match &self.command {
            ...
            SWDCommand::Read { start_address: _, length: data_length } => {
                match data[Self::ACK_OFFSET] {
                    Self::READ_ACK => {
                        let result =
                            data[
