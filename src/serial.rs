...
pub fn connect_to_device() -> Result<Box<dyn SerialPort>, Box<dyn std::error::Error>> {
    ...
    println!("Device with PID 0x{:04X} found", target_pid);
    ...
    for port in &ports {
        ...
        if let SerialPortType::UsbPort(usb_info) = &port.port_type {
            if usb_info.pid == target_pid {
                ...
                println!("Found matching USB serial port: {}", port.port_name);
                break;
            }
        }
    }
    ...
    let port_name = target_port.unwrap_or_else(|| {
        println!("No matching USB port found, using first available port: {}", ports[0].port_name);
...
pub fn write_to_device(port: &mut Box<dyn SerialPort>, data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
    // Send the data
    port.write_all(data)?;
    port.flush()?;
...
pub fn read_from_device(port: &mut Box<dyn SerialPort>, buffer_size: usize, timeout_ms: u64) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    // Set read timeout
    ...
    match port.read(&mut buffer) {
        Ok(bytes_read) => {
            if bytes_read > 0 {
                ...
                println!("Received {} bytes: {:02X?}", bytes_read, &buffer[..bytes_read]);
            } else {
        ...
        Err(e) => {
            ...
            if e.kind() == std::io::ErrorKind::TimedOut {
                println!("Read timeout - no data received from ESP32");
            } else {
                return Err(e.into());
            }
        }
...
pub fn write_and_read(port: &mut Box<dyn SerialPort>, data: &[u8], read_buffer_size: usize, read_timeout_ms: u64) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    // Send data first
...
pub fn send_debug_sequence_and_read(port: &mut Box<dyn SerialPort>) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let debug_data: [u8; 8] = [0xff, 0xf9, 0x00, 0x02, 0xc2, 0x00, 0xf5, 0xe7];
